...
WEBOSCKET_SERVICE_PORT = int(os.getenv("WEBOSCKET_SERVICE_PORT"))
...
class ZexBot:
    ...
    def send_order_transaction(self, price, verbose):
        ...
        tx = self.create_order(price, volume, verbose)
    ...
    def run(self):
        ...
        self.websocket = WebSocketApp(
            f"ws://{HOST}:{WEBOSCKET_SERVICE_PORT}/v1/ws",
        ...
        while self.is_running:
            maker = self.rng.choices([True, False], [0.4, 0.6])[0]
            price = 0
            if maker:
                if self.side == "buy":
                ...
                if self.side == "buy":
                    price = self.mark_price + 10 * 10**-self.price_digits
                elif self.side == "sell":
                    price = self.mark_price - 10 * 10**-self.price_digits
            self.send_order_transaction(price, verbose=False)
            time.sleep(1)
