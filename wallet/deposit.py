...
def get_deposits(block):
    ...
    events = contract.events.Deposit.createFilter(fromBlock=block, toBlock=block).get_all_entries()
...
def create_tx(deposits, from_block, to_block, timestamp):
    version = pack('B', 0)
    chain = b'bst'
    from_block = pack('Q', from_block)
    to_block = pack('Q', to_block)
    tx = version + b'd' + chain + from_block + to_block + pack('H', len(deposits))
    for deposit in deposits:
        prefix = '02' if deposit['pubKeyYParity'] == 0 else '03'
        pubKeyX = f"{deposit['pubKeyX']:#0{32}x}"[2:]
        public = (prefix + pubKeyX).encode()
...
def main():
    # processed_block = web3.eth.blockNumber - BLOCKS_CONFIRMATION # should be queried from zex
    processed_block = 39228676
    ...
    while True:
        ...
        block = web3.eth.getBlock(processed_block)
        tx = create_tx(deposits, sent_block + 1, processed_block, block['timestamp'])
        # requests.post(SEND_URL, data={ 'tx': tx })
